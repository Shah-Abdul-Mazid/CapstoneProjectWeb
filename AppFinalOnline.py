...
# # ==================== MODEL URLS FROM YOUR GITHUB REPO ====================
# MODEL_URLS = {
#     "Combined Dataset (Balanced)": "https://github.com/Shah-Abdul-Mazid/CapstoneProjectWeb/raw/main/models/DatasetCombined/Balance/Hybrid_MobDenseNet_CBAM_GradCAM.h5",
...
class ChannelAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.shared_dense_one = tf.keras.layers.Dense(channels // self.ratio, activation='relu', kernel_initializer='he_normal', use_bias=True)
        self.shared_dense_two = tf.keras.layers.Dense(channels, kernel_initializer='he_normal', use_bias=True)
    ...
    def call(self, inputs):
        ...
        attention = tf.keras.layers.Reshape((1, 1, tf.shape(inputs)[-1]))(attention)
...
class SpatialAttention(Layer):
    ...
    def build(self, input_shape):
        self.conv = tf.keras.layers.Conv2D(1, kernel_size=self.kernel_size, strides=1, padding='same',
                                           activation='sigmoid', use_bias=False, kernel_initializer='he_normal')
        super().build(input_shape)
...
# ==================== MODEL URLS (FROM YOUR GITHUB) ====================
MODEL_URLS = {
    "Combined Dataset (Balanced)": 
        "https://raw.githubusercontent.com/Shah-Abdul-Mazid/CapstoneProjectWeb/main/models/DatasetCombined/Balance/Hybrid_MobDenseNet_CBAM_GradCAM.h5",
    
    "Dataset 002 (Balanced)": 
        "https://raw.githubusercontent.com/Shah-Abdul-Mazid/CapstoneProjectWeb/main/models/Dataset002/Balance/FinalModel/Hybrid_MobDenseNet_CBAM_GradCAM.h5",
    
    "Dataset 001 (Balanced)": 
        "https://raw.githubusercontent.com/Shah-Abdul-Mazid/CapstoneProjectWeb/main/models/Dataset001/Balance/Hybrid_MobDenseNet_CBAM_GradCAM.h5",
...
# ==================== CACHED MODEL LOADER ====================
@st.cache_resource(show_spinner="Downloading & loading model (200–400 MB)... First load takes 60–90s")
def load_brain_model(url: str):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".h5") as tmp:
        tmp_path = tmp.name
        ...
        st.success("Model downloaded! Loading into memory...")
        model = load_model(tmp_path, custom_objects=custom_objects, compile=False)
        os.unlink(tmp_path)
        return model
...
def overlay_heatmap(img_pil, heatmap, alpha=0.5):
    ...
    heatmap = np.uint8(255 * heatmap)
    heatmap_color = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
    superimposed = cv2.addWeighted(np.array(img_pil), 1 - alpha, heatmap_color, alpha, 0)
    return Image.fromarray(superimposed)

# ==================== PAGE CONFIG & CSS ====================
st.set_page_config(page_title="Brain Tumor MRI Classification — Thesis", page_icon="brain", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    .card {background:#FFFFFF; border-radius:10px; padding:18px; box-shadow:0 2px 8px rgba(15,23,42,0.06); margin-bottom:12px;}
    .small {font-size:14px; color:#222; line-height:1.5;}
    .footer {text-align:center; padding:14px; background:#0B1B2B; color:white; border-radius:8px;}
    .stButton>button {background:#0B4F6C; color:white;}
...
# ==================== SIDEBAR ====================
with st.sidebar:
    st.markdown("<div style='text-align:center'><img src='https://img.icons8.com/ios-filled/100/000000/brain.png' width='80'></div>", unsafe_allow_html=True)
    selected = option_menu(
        ...
        default_index=0,
        styles={"nav-link-selected": {"background-color": "#E8F0FE"}}
    )
...
if selected == "Home":
    ...
    st.markdown("This thesis project implements a hybrid convolutional neural network...", unsafe_allow_html=True)
    col1, col2 = st.columns([2, 3])
    with col1:
        st.image("https://img.freepik.com/free-photo/doctor-holding-mri-brain-scan_23-2149366759.jpg", caption="MRI Scan")
    with col2:
        st.markdown("### Key Contributions\n- Hybrid backbone\n- CBAM attention\n- Grad-CAM++ explainability", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    model_choice = st.selectbox("Select trained model", list(MODEL_URLS.keys()))
    model = load_brain_model(MODEL_URLS[model_choice])
    ...
    if uploaded:
        ...
        border_color = "#0B63D6" if pred_class == "No Tumor" else "#C62828"
        st.markdown(f"""
        <div style="padding:18px; border-left:6px solid {border_color}; border-radius:8px; background:white; color:black;">
            <div style="font-size:16px"><strong>Prediction</strong></div>
            <div style="font-size:28px; font-weight:700;">{pred_class}</div>
            <div>Confidence: <strong>{confidence:.2f}%</strong> • Time: {inference_time:.3f}s</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    model_choice = st.selectbox("Select model", list(MODEL_URLS.keys()), key="gc")
    layer_name = st.text_input("Last conv layer name", DEFAULT_LAST_CONV_LAYER)
    model = load_brain_model(MODEL_URLS[model_choice])
    
    uploaded = st.file_uploader("Upload MRI", type=["jpg", "png", "jpeg"], key="gc_up")
    if uploaded:
        img = Image.open(uploaded).convert("RGB")
        img_array = preprocess_image(img)
        ...
        with col2: st.image(overlay, caption=f"Grad-CAM → {pred_class}")
        
        st.download_button("Download Heatmap", pil_image_to_bytes(overlay), "gradcam.png", "image/png")
    st.markdown("</div>", unsafe_allow_html=True)

# Add other pages (Methods, Results, Dataset) similarly...
# (I've kept them short for brevity — you can copy from your original)

# ==================== FOOTER ====================
st.markdown("---")
st.markdown("""
<div class="footer">
    <h4>Master's Thesis Project • 2025</h4>
    <div>Hybrid Deep Learning for Brain Tumor Detection with Grad-CAM++</div>
    <div>Shah Abdul Mazid • Department of Computer Science & Engineering</div>
</div>
""", unsafe_allow_html=True)